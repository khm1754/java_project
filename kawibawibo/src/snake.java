...
public class snake extends JFrame {
		static class XY {
		 ...
		 JPanel panelCenter;
		 JLabel labelTitle;
		 JLabel labelMessage; //250ms * 40칸
		 JPanel[][] panels = new JPanel[20][20];
		 int[][] map = new int[20][20]; //과일 위치 9, 폭탄 8
		 LinkedList<XY> snake = new LinkedList<XY>();
		 int dir = 3; //진행방향 0:up,1:down
		                    //2:left,3:right
		 int score = 0; //점수
		 int time = 0; //진행시간
		 int timeCount = 0; //200ms마다 카운트를 센다
		 Timer timer = null; //뱀이 움직임 처리,
		                            //판정(열매,충돌)
